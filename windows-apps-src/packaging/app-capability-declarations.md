...
| **視線トラッカー** | **gazeInput** 機能を使うと、互換性のある視線追跡デバイスが接続されているときのアプリケーション境界内で、または視線追跡がサポートされている Mixed Reality デバイスで、ユーザーがどこを見ているかを検出できます。 [**Windows.Devices.Input.Preview**](/uwp/api/windows.devices.input.preview) 名前空間の一部の API を使用する場合は、この機能が必要になります。 Mixed Reality デバイスの場合、[**Windows.Perception.People.EyesPose**](/uwp/api/windows.perception.people.eyespose) 内の API に、この機能が必要です。 |
