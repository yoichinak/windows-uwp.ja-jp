...
ホストされているアプリは、コンポーネント (実行可能ファイルやスクリプトファイルなど) が Windows 10 アプリのように動作するようにする場合に便利ですが、コンポーネントを実行するにはホストプロセスが必要です。 たとえば、PowerShell スクリプトや Python スクリプトを、実行するためにホストをインストールする必要があるホスト型アプリとして配信することができます。 ホストされているアプリは、独自のスタートタイル、id、および Windows 10 の機能 (バックグラウンドタスク、通知、タイル、共有ターゲットなど) との緊密な統合を持つことができます。
