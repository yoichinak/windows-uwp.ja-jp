...
Windows 10 バージョン1607では、次の2つのアプリモデルの状態が導入されています。**フォアグラウンドで実行**し、**バックグラウンドで実行**します。 以下のセクションでこれらの新しい状態についても説明します。
