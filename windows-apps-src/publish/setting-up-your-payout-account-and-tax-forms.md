...
### <a name="to-set-up-your-payout-account"></a>受取りアカウントを設定するには
 ...
 **PayPal アカウントは、パートナー センター アカウントと同じ国/リージョンに登録することがあるでしょうか。** No. PayPal アカウントを設定する際は、既定の構成をそのまま使用できます。 特定の通貨による支払いをブロックしていない限り、国/地域と通貨が違っていても、問題は発生しません。 この設定は、PayPal の [支払い受領設定] ページで管理します。
- **PayPal の支払いを手動でそのまま使用する必要があるでしょうか。** No. PayPal アカウントは、既定では、ユーザーに対して支払いを手動で承認するように求める設定になっています。この場合は、支払いを 30 日以内に承認しないと、支払いが返されることになります。 この設定を変更するには、PayPal の [その他の設定] ページで [確認する] をオフにします。
...
###<a name="to-complete-your-tax-profile"></a>税務プロファイルを作成するには
