---
Description: パートナーセンターのレビューレポートでは、ストアでアプリを評価するときに顧客が入力したレビュー (コメント) を確認できます。
title: レビュー レポート
...
ms.locfileid: "79210988"
---
# <a name="reviews-report"></a>レビュー レポート


[パートナーセンター](https://partner.microsoft.com/dashboard)の**レビュー**レポートでは、ストアでアプリを評価するときに顧客が入力したレビュー (コメント) を確認できます。
...
また、[このページから直接](respond-to-customer-reviews.md)、または MICROSOFT STORE review API を[使用してプログラムで](../monetize/submit-responses-to-app-reviews.md)顧客レビューに応答することもできます。
...
- **[内容の確認]** : 既定の設定では [**評価コンテンツの評価] と**表示されます。これは、レビューコンテンツが含まれる評価のみが表示されることを意味します。 すべての評価を表示するには、 **[すべて]** を選択します。書面によるレビューのテキストが含まれていない場合でも同様です。 **評価内訳**グラフには、選択内容に関係なく、常にすべてのレビューが表示されることに注意してください。
...
-   **[カテゴリ名]** : 既定のフィルターは **[すべて]** です。 特定の[レビューインサイトカテゴリ](#review-insight-categories)を選択すると、そのカテゴリに関連付けられているレビューのみが表示されます。 
...
次の内容を簡単に確認できるように、このレポートの一番上に**評価内訳**グラフが表示されます。 
- アプリの平均の星評価。
...
**[コンテンツの確認]** ページフィルターで **[レビューコンテンツの評価]** を選択した場合でも、このグラフには常にすべてのレビューが含まれていることに注意してください。

このグラフは、アプリの評価に関する詳細情報と共に、[評価レポート](ratings-report.md)にも表示されます。


<span id = "review-insight-categories" />

## <a name="insight-categories"></a>洞察のカテゴリ

**洞察カテゴリ**グラフでは、レビューに関連付けられていると判断したカテゴリに基づいてレビューがグループ化されます。
...
-   レビューの日付。
-   Microsoft Store に表示されるレビュー担当者の名前。
-   レビューアーの国/地域。
-   レビューが残された時点でのユーザーのデバイス上のアプリのパッケージ バージョン (この情報は、オンラインで送信されたレビューまたは Windows 8.1 以前の顧客によって送信されたレビューには使用できません)。
-   レビューが残されたときにユーザーが使っていたデバイスの OS のバージョン。
-   レビューが残されたときにユーザーが使っていたデバイスの名前 (この情報は、オンラインで送信されたレビューまたは Windows 8.1 以前の顧客によって送信されたレビューには使用できません)。
...
このページのレビューは、日付や評価によって昇順または降順に並べ替えることができます。 **[並べ替え]** リンクをクリックして、**日付**や**評価**で並べ替えるためのオプションを表示します。

検索ボックスを使用して、アプリのレビューで特定の単語や語句を検索することもできます。 レビューが別の言語で記述されている場合でも、顧客によって書き込まれた元のレビューテキストのみが検索されることに注意してください。 翻訳されたレビューテキストは検索されません。
...
前述のように、レビューの検索時には、 **[翻訳]** チェックボックスがオンになっている場合でも、顧客が残した元のテキストだけが検索されます (翻訳されたテキストは検索されません)。


## <a name="responding-to-customer-reviews"></a>顧客のレビューへの返信

[パートナーセンター](https://partner.microsoft.com/dashboard)または[Microsoft Store レビュー API](../monetize/submit-responses-to-app-reviews.md)を使用して、顧客の多くのレビューに応答を送信できます。 詳しくは、「[顧客のレビューに返信する](respond-to-customer-reviews.md)」をご覧ください。
