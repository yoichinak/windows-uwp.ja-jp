...
パッケージフライトでは、パッケージのみが異なります。ストアの一覧の詳細は、すべてのお客様に対して同じになります。 フライトグループ内のすべてのユーザーは、パッケージフライトに含まれるパッケージを受け取りますが、フライトグループに属していないお客様は、引き続き通常の (無光) パッケージを受け取ります。  パッケージフライトのパッケージをすべての顧客が使用できるようにすることを後で決定する場合は、これらの同じパッケージを、非光の送信で簡単に使用できます。 パッケージフライトは、送信と同様に [認定プロセス](the-app-certification-process.md)に合格する必要があることに注意してください。
...
パッケージを指定し、必要なオプションを構成したら、 **[ストアに提出]** をクリックします。 パッケージ フライトに対して、[アプリの認定プロセス](the-app-certification-process.md)が開始されます。 パッケージフライトに含まれるパッケージは、すべての送信の場合と同様に、 [Microsoft Store ポリシー](store-policies.md)に準拠する必要があることに注意してください。
