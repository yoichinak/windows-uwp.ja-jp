...
>**メモ**  この機能は、.NET の使用のみを対象としています。 クライアント アプリとデスクトップ コンポーネントの両方が、.NET を使って作成されている必要があります。
