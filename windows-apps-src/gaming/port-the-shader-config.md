...
定数バッファー要素には DirectXMath XM 型を使用し \* ます。これは、シェーダーパイプラインに送信されるときに、コンテンツのパッキングとアラインメントが適切に行われるためです。 Windows プラットフォームの標準の浮動小数点型と配列を使う場合は、手動でパッキングとアラインメントを実行する必要があります。
