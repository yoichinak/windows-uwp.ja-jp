...
Microsoft Store を使用すると、世界中の200の国や地域でゲームをダウンロードできるようになり、Visa、MasterCard、PayPal などのさまざまな形式の支払いによる課金がサポートされるようになります。 国と地域の完全な一覧については、「[マーケット選択の定義](https://docs.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection)」を参照してください。
...
ゲームの基本価格は、提供するゲームが_有料_か_無料_かを決定する要素です。 [パートナーセンター](https://partner.microsoft.com/dashboard)を使用して、国と地域に基づいて基本料金を構成できます。
...
ゲーム内購入とは、ゲームの中で購入できる製品です。 これらの製品は、一般に_アプリ内購入_とも呼ばれます。 この Microsoft Store では、これらの製品を_アドオン_と呼びます。 アドオンは、パートナーセンターを通じて[公開され](https://docs.microsoft.com/windows/uwp/publish/add-on-submissions)ます。 また、ゲームのコードでアドオンを有効にする必要があります。
...
また、パートナーセンターとストアでサポートされている将来の種類の製品や機能と互換性があるように設計されています。 以前のバージョンの Windows 10 向けに開発を行う場合は、この代わりに [Windows.ApplicationModel.Store](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store) 名前空間を使います。
...
ゲームに多くのアドオンがある場合は、 __Microsoft Store 送信 API__を使用してプログラムで作成することができます。 詳細については、「 [Microsoft Store services を使用した送信の作成と管理](https://docs.microsoft.com/windows/uwp/monetize/create-and-manage-submissions-using-windows-store-services)」を参照してください。
...
* ネイティブ広告 &mdash;、アプリケーションに統合できる個々の要素として、ad クリエイティブの各部分 (タイトル、画像、説明、アクション呼び出しのテキストなど) がアプリに配信されるコンポーネントベースの広告です。
...
* [XAML および .NET でバナー広告を作成する](https://docs.microsoft.com/windows/uwp/monetize/adcontrol-in-xaml-and--net)
* [HTML5 および JavaScript でバナー広告を作成する](https://docs.microsoft.com/windows/uwp/monetize/adcontrol-in-html-5-and-javascript)
* [スポット広告を作成する](https://docs.microsoft.com/windows/uwp/monetize/interstitial-ads)
* [ネイティブ広告を作成する](https://docs.microsoft.com/windows/uwp/monetize/native-ads)
...
* [バナー広告のベストプラクティス](https://docs.microsoft.com/windows/uwp/monetize/ui-and-user-experience-guidelines)
...
* [アカウントの種類、場所、料金](https://docs.microsoft.com/windows/uwp/publish/account-types-locations-and-fees)
...
* [アプリの試用版を実装する](https://docs.microsoft.com/windows/uwp/monetize/implement-a-trial-version-of-your-app)
* [A/B テストを使用してアプリの実験を実行する](https://docs.microsoft.com/windows/uwp/monetize/run-app-experiments-with-a-b-testing)
