...
次に、プロジェクトのプロパティ ( **[全般]** \> **[ターゲット プラットフォーム バージョン]** ) が 10.0.17134.0 (Windows 10 バージョン 1803) 以上に確実に設定されるようにします。
...
空の参照の配列を作成するための簡単な方法はありません。 配列内の要素ごとに、`nullptr` を繰り返す必要があります。 値が少なすぎる場合は、追加分が既定で構築されます。
