...
<td align="left"><strong>Core location</strong> <strong>cllocationmanager</strong>クラスは、デバイスの場所を監視するために使用されます。このクラスは、標準ロケーションサービスの場合は<strong>startupdatinglocation</strong> 、<strong>重要な変更</strong>位置サービスの場合は<strong>startMonitoringSignificantLocationChanges</strong>です。</td>
