...
| _XamlCompiler エラー WMC0055:テキスト値を割り当てることはできません '&lt;ストリーム ジオメトリ&gt;' プロパティの型 'RectangleGeometry' の ' クリップ' に_ | UWP では、[Microsoft DirectX](https://docs.microsoft.com/windows/desktop/directx) と XAML C++ UWP アプリの型を使います。 |
| _XamlCompiler エラー WMC0001:不明な型 'RadialGradientBrush' XML 名前空間 [...]_ | UWP には、**RadialGradientBrush** 型がありません。 マークアップから **RadialGradientBrush** を削除し、[Microsoft DirectX](https://docs.microsoft.com/windows/desktop/directx) と XAML C++ UWP アプリのその他の型を使います。 |
| _XamlCompiler エラー WMC0011:不明なメンバー 'OpacityMask' 要素に '&lt;UIElement 型&gt;'_ | UWP の  [Microsoft DirectX](https://docs.microsoft.com/windows/desktop/directx) と XAML C++ UWP アプリを使います。 |
| _システムで型 'System.Runtime.InteropServices.COMException' の初回例外が発生しました。NI します。DLL です。追加情報:アプリケーションでは、別のスレッドにマーシャ リングされたインターフェイスが呼び出されます。(HRESULT からの例外: 0x8001010E (RPC_E_WRONG_THREAD))._ | ここで実行する作業は、UI スレッド上で行う必要があります。 [  **CoreWindow.GetForCurrentThread**](https://docs.microsoft.com/uwp/api/windows.ui.core.corewindow.getforcurrentthread)) を呼び出します。 |
