---
description: A/B テストを使用してユニバーサル Windows プラットフォーム (UWP) アプリで実験を実行する前に、パートナーセンターで実験を定義する必要があります。
...
ms.locfileid: "93033215"
---
# <a name="define-your-experiment-in-partner-center"></a>パートナー センターで実験を定義する

[プロジェクトを作成し、パートナーセンターでリモート変数を定義](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)し、[実験のためにアプリをコーディング](code-your-experiment-in-your-app.md)したら、プロジェクトに実験を作成する準備が整います。 実験を作成するときは、ゴールとユーザーが受け取るバリエーションを定義します。
...
8.   [プロジェクトID](run-app-experiments-with-a-b-testing.md#terms) の値をメモしておきます。 [実験用にアプリをコーディング](code-your-experiment-in-your-app.md)する場合は、コードでこの ID を参照する必要があります。これにより、バリエーションデータとレポートビューおよび変換イベントをパートナーセンターに受信できるようになります。
...
> [!NOTE]
> パートナーセンターは、24時間以内に各ユーザービューの最初の変換イベントのみを報告します。 ユーザーが 24 時間以内にアプリで複数のコンバージョン イベントをトリガーした場合は、最初のコンバージョン イベントのみ報告されます。 これは、コンバージョンを行ったユーザーの数を最大化することを目標としていた場合に、単一のユーザーによって、サンプルのユーザー グループの試験的機能の結果が歪曲されないようにすることを目的としています。
...
試験的機能のパラメーターに問題がなく、試験的機能をアクティブ化する準備ができたら、 **[アクティブ化]** をクリックして、アプリから試験的機能データの収集を開始することができます。 実験がアクティブになると、アプリはバリエーション変数とレポートビューおよび変換イベントをパートナーセンターに取得できます。 詳細については、「 [パートナーセンターでの実験の実行と管理](manage-your-experiment.md)」を参照してください。
...
パートナーセンターで実験を定義し、アプリで実験をコーディングしたら、 [パートナーセンターで実験を実行して管理](manage-your-experiment.md)する準備ができました。
