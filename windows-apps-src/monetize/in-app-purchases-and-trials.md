...
1. 場合は、アプリは、顧客が購入できるアドオン[パートナー センターでのアプリのサブミッションをアドオンの作成](https://docs.microsoft.com/windows/uwp/publish/add-on-submissions)です。
...
1. 場合は、アプリではまだ公開されて、使用可能なストア、アプリが、最小値を満たしている確認[Windows アプリ認定キット](https://developer.microsoft.com/windows/develop/app-certification-kit)要件、[アプリ送信](https://docs.microsoft.com/windows/uwp/publish/app-submissions)パートナー センターでアプリを確認して認定プロセスを渡します。 テスト中に[ストアでアプリを検索できないようにアプリを構成する](https://docs.microsoft.com/windows/uwp/publish/set-app-pricing-and-availability)ことも可能です。 適切な構成に注意してください[フライトをパッケージ化](../publish/package-flights.md)します。 正しく構成されているパッケージのフライト可能性がありますできませんをダウンロードします。
...
2. 次に、以下の操作が完了していることを確認します。
    ...
    * 場合は、アプリは、顧客が購入できるアドオン[パートナー センターで、アプリ、アドオン サブミッションを作成](https://docs.microsoft.com/windows/uwp/publish/add-on-submissions)です。
...
アプリで **Windows.ApplicationModel.Store** 名前空間を使用している場合は、アプリで [CurrentAppSimulator](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentAppSimulator) クラスを使用することで、アプリをストアに提出する前に、ライセンス情報をテスト中にシミュレートできます。 詳細については、次を参照してください。 [CurrentApp と CurrentAppSimulator クラスの概要](in-app-purchases-and-trials-using-the-windows-applicationmodel-store-namespace.md#get-started-with-the-currentapp-and-currentappsimulator-classes)します。  
