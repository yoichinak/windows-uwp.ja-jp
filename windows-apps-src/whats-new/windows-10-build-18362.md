...
デスクトップ アプリケーションの UWP コントロール (XAML アイランド) | WPF、Windows フォーム、および C++ Win32 デスクトップ アプリケーションで UWP コントロールをホストするための Windows SDK の API は、開発者向けプレビューに表示されなくなりました。 詳細については、[デスクトップ アプリケーションでの UWP コントロール](../xaml-platform/xaml-host-controls.md)に関するページを参照してください。
