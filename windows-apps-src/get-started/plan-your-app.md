...
-   **近接ジェスチャ**  ユーザーが物理的に近接、物理的にまとめて、デバイスをタップして (マルチ プレーヤー ゲーム) の他のユーザーとデバイスを接続できるようにします。 詳しくは、「[近接通信とタップのサポート](https://docs.microsoft.com/previous-versions/windows/apps/hh465229(v=win.10))」をご覧ください。
-   **カメラおよび外部記憶装置**  チャットと会議、vlogs の世界を文書化、プロファイル写真の取得を記録する、組み込みまたは接続されているカメラにユーザーを接続または任意のアクティビティ、アプリ点で優れています。 詳しくは、[リムーバブル ストレージのコンテンツへのアクセス](https://docs.microsoft.com/previous-versions/windows/apps/hh465189(v=win.10))についてのページをご覧ください。
...
-   **地理的位置情報**  を回避、地図上の位置を検索または近くの人、アクティビティ、および変換先の通知を受け取るため、ユーザーを標準的な web データや地理的位置情報センサーからの地理的位置情報の情報を使用しています。 詳しくは、[地理位置情報](https://docs.microsoft.com/previous-versions/windows/apps/hh465139(v=win.10))についてのページをご覧ください。
...
-   **場合は、コンテンツを使用することはできません。** コンテンツを直接操作できない場合は、コマンドを次の UI サーフェスのいずれかに配置します。
    ...
    -   [コマンド バー](https://docs.microsoft.com/windows/uwp/controls-and-patterns/app-bars):ほとんどのコマンドは、ユーザーがタップして表示されるまで、通常は隠されているコマンド バーに配置する必要があります。
    -   アプリのキャンバス。ページまたはビューを 1 つの目的を持つユーザーにある場合は、そのため、キャンバス上で直接コマンドを行うことができます。 しかし、このようなコマンドはなるべく作らないでください。
    -   [コンテキスト メニュー](https://docs.microsoft.com/windows/uwp/controls-and-patterns/menus):クリップボード操作 (切り取り、コピー、および貼り付け) などのコンテキスト メニューを使用するまたは適用されるコマンドのコンテンツは選択できません (など、マップ上の場所へのピンの形の追加) します。
