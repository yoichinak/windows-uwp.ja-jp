...
プログラムによる手法は、ユーザーの優先される表示言語の `LayoutDirection` プロパティを使用して [**FlowDirection**](/uwp/api/Windows.UI.Xaml.FrameworkElement.FlowDirection) プロパティを設定することです (以下のコード例を参照)。 Windows のほとんどのコントロールでは、 **system.windows.flowdirection>** が既に使用されています。 カスタム コントロールを実装する場合は、 **FlowDirection** を使って RTL 言語と LTR 言語で適切なレイアウト変更を行う必要があります。
