---
Description: このチュートリアルでは、基本的なアプリケーションユーザーインターフェイスを作成する方法について説明します。 最も一般的な 2 つの XAML 要素 Grid と StackPanel の使用方法を紹介します。
title: Grid と StackPanel を使用して、簡単なアプリを作成します。
template: detail.hbs
ms.date: 05/19/2017
ms.topic: article
...
- Windows 10 および Microsoft Visual Studio 2015 以降。 (最新の Visual Studio は、最新の開発およびセキュリティ更新プログラムに推奨)[Visual Studio でセットアップする方法については、ここをクリックして](../../get-started/get-set-up.md)ください。
...
この新しい **Grid** によって 2 行 x 2 列のセットが作成され、このアプリ インターフェイスのレイアウトが定義されます。 最初の列の幅は "3\*" で、2番目の列の**幅**は "5\*" で、2つの列の間の左右の間隔は3:5 の比率で除算されます。 同様に、2つの行の**高さ**はそれぞれ "2\*" と "\*" になります。そのため、**グリッド**は2つ目の行に対して2倍の領域を割り当てます ("\*" は "1\*" と同じ)。 これらの比率は、ウィンドウのサイズを変更したり、異なるデバイスを使ったりしても変わりません。
