...
***一般的な相互作用パターンは、キーボードとゲームパッドの間で共有されます。***

このトピックでは、PC におけるキーボード入力の UWP アプリの設計について重点的に説明します。 ただし、タッチキーボードやスクリーンキーボード (OSK) などの[ソフトウェアキーボード](#software-keyboard)を使用したり、Xbox ゲームパッドやリモコンのような他の入力デバイスを処理したりするために、適切に設計されたキーボードエクスペリエンスは、Windows ナレーターなどのユーザー補助ツールをサポートするために重要です。
...
## <a name="built-in-support"></a>組み込みのサポート
...
| デバイス/ツール                       | 説明     |
|-----------------------------------|-----------------|
|キーボード (ハードウェアおよびソフトウェア)   |UWP アプリケーションでは、標準のハードウェアキーボードに加えて、[タッチ (またはソフトウェア) キーボード](#software-keyboard)と[スクリーンキーボード](#on-screen-keyboard)という2つのソフトウェアキーボードがサポートされています。|
...
### <a name="focus-visuals"></a>ビジュアルのフォーカス

UWP は、あらゆる入力の種類とエクスペリエンスで適切に動作する 1 つのフォーカスの視覚効果デザインをサポートしています。
visual](images/keyboard/focus-visual.png) にフォーカスを ![
...
既定のタブの順序をオーバーライドするには、 [TabIndex](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex)プロパティを設定します。
...
UWP アプリの場合、最初のフォーカスは、フォーカスを受け取ることができる最も高い[TabIndex](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex)を持つ要素に設定されます。 コンテナー コントロールの子要素は無視されます。 TIE の場合、表示ツリー内の最初の要素がフォーカスを受け取ります。

#### <a name="set-initial-focus-on-the-most-logical-element"></a>初期フォーカスは、最も論理的な要素に設定します。

初期フォーカスは、ユーザーがアプリを起動したり、ページを移動したりするときに最初に実行する可能性が最も高い操作 (つまり、プライマリ操作) の UI 要素に設定します。 たとえば、次のものがあります。
...
-   **方向キー**を使うと、コントロール固有の "内部ナビゲーション" が表示されます。ユーザーが "内部ナビゲーション" に入ると、方向キーを使ってもコントロール外に移動しません。 たとえば、次のものがあります。
    -   上方向キーまたは下方向キーを押すと、`ListView` と `MenuFlyout` 内でフォーカスが移動します
    -   現在選択されている `Slider` と `RatingsControl` の値を変更する
    -   `TextBox` 内でカレットを移動します
    -   `TreeView` 内の項目を展開する/折りたたむ
...
***矢印キーを使用すると、関連するボタンのコレクションとの対話が容易になります。***
...
***2つの積み上げられた ListView コントロールとの相互作用は、タブストップを排除し、方向キーだけで移動することで、より簡単かつ効率的にすることができます。***
...
| 操作                               | キー コマンド                                      |
|--------------------------------------|--------------------------------------------------|
| すべて選択                           | Ctrl&lt;/localizedText&gt; + &lt;localizedText&gt;A                                           |
| 連続して選択                  | Shift + 方向キー                                  |
| 保存                                 | Ctrl + S                                           |
| [検索]                                 | Ctrl + F                                           |
| [印刷]                                | Ctrl + P                                           |
...
<table>
  <tr>
    ...
    <td>
      ...
      <p><sup>ListItems と GridItems</sup></p>
    </td>    
  </tr>
</table>
...
たとえば、次のものがあります。
-   `RadioButtons`
-   複数の `ListViews` が表示され、1つの `ListView` のように動作する
...
#### <a name="directional-navigation"></a>方向のナビゲーション
...
<table>
  <tr>
    <td>
      ...
      <p><em><strong>方向のナビゲーションがサポートされてい</strong>ます</br>方向キー (キーボードの矢印、ゲームパッド、リモートコントロール D パッド) を使用すると、ユーザーはさまざまなコントロール間を移動できます。</em></p>
    </td>
    <td>
...
### <a name="built-in-keyboard-optimization"></a>組み込みのキーボード最適化
...
### <a name="software-keyboard"></a>ソフトウェアキーボード
...
タッチキーボードでは、トグルとほとんどのシステムコマンドがサポートされてい**ない  ます**。
