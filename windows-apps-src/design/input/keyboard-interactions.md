...
(コンテキスト メニュー、コマンド バー オーバーフロー メニュー、AutoSuggest メニューなど) の一部のコントロールは、プライマリ コントロールおよび可能な画面領域と比較 (既定) では下位の位置と方向メニュー ポップアップを表示します。 実行時に開く方向がさまざまな要因によって影響を受けることに注意してください。
...
これらのコントロールのメニューが開いた最初 (と、ユーザーが項目が選択されていません)、下矢印キー常にフォーカス設定最初の項目に上向きの矢印キー常にフォーカスを設定、最後に項目メニューの中にします。 

最後の項目にフォーカスがあり、下矢印キーが押された場合、メニューの最初の項目にフォーカスが移動します。 同様に、最初の項目にフォーカスがある、上向きの矢印キーを押すと、フォーカスが移動最後の項目、メニュー。 この動作と呼ばれます*サイクリング*を開くことができる予測不可能な方向にポップアップ メニューを移動するために便利です。

> [!NOTE]
> 循環は非ポップアップ Ui が無限ループにトラップをユーザーがどこに避ける必要があります。 

カスタム コントロールでこれらの同じ動作をエミュレートすることをお勧めします。 この動作を実装する方法のコード サンプルが記載[プログラムによるフォーカスのナビゲーション](focus-navigation-programmatic.md#find-the-first-and-last-focusable-element)ドキュメント。
