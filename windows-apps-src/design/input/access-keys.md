...
_Microsoft Word のアクセスキーの KeyTip バッジ_
...
microsoft Word @ no__t の @no__t 0 のプライマリアクセスキー Microsoft word_のプライマリアクセスキー_

microsoft Word での @no__t 0Secondary のアクセスキー @ no__t-1_セカンダリアクセスキー (Microsoft word_ )
...
_リソース. resw ファイルで指定されている AccessKey プロパティを指定します_
...
1.  **明確な関連付けの原則**:ユーザーは、コントロールをキーヒントに簡単に関連付けることができます。
...
2.  **発見**可能性:ユーザーは、キーヒントを使用してコントロールをすばやく見つけることができます。

    a.  キーのヒントが他のキーのヒントと**重なる**のを回避してください。  

3.  **簡単なスキャン:** ユーザーは、重要なヒントを簡単にざっと見ることができます。
...
配置モードは次のとおりです。Top、Bottom、Right、Left、Hidden、Center、および Auto。
