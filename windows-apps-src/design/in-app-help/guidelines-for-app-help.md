...
-   **理解しやすい:** ヘルプがまったく役に混乱させことはありません。

-   **簡単**ヘルプを探しているユーザーは、回答をクリアしたいと考えています。

-   **関連**ユーザーは、特定の問題を検索する必要はありません。 ユーザーはその場面に最も関連があって適切なヘルプ (コンテキスト ヘルプと呼ばれます) がすぐに表示されることを求めています。または容易なナビゲーションのためのインターフェイスが必要です。

-   **接続**ユーザーがヘルプを検索すると、ヘルプが表示されます。 アプリのヘルプの中に、バグの報告、フィードバックの送信、サービス規定の表示などの機能へのリンクが含まれていてもかまいませんが、 それらはメインのヘルプ ページの補足として含むものであり、メインのヘルプのコンテンツとなるものではありません。

-   **・** 種類に関係なく、ヘルプは依然としてアプリの一部であり、UI の他の部分として扱う必要があります。 アプリの他の部分と同じ設計原則による、操作性、アクセシビリティ、スタイルが、ヘルプでも採用されている必要があります。
...
[指示 UI のガイドラインを参照してください。](instructional-ui.md)
...
[アプリ内ヘルプのガイドラインを参照してください。](in-app-help.md)
