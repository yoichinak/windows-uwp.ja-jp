...
ユーザーが英語またはドイツ語以外の言語を話す場合、または 100% または 300% 以外の表示スケールファクターを使用している場合、MRT.DLL はフォールバック規則のセットに基づいて "最も近い" 一致候補を選択します (詳細については、「[リソース管理システム](https://docs.microsoft.com/previous-versions/windows/apps/jj552947(v=win.10))」を参照してください)。

MRT.DLL は複数の修飾子に合わせて調整されたリソースをサポートすることに注意してください。たとえば、ロゴ画像にローカライズが必要な埋め込みテキストが含まれている場合、ロゴには次の4つの候補があります。EN/Scale-100、DE/Scale-100、EN/Scale-300、DE/Scale-300。

### <a name="sections-in-this-document"></a>このドキュメントのセクション

以下のセクションでは、アプリケーションを MRT と統合するために必要なタスクの概要を説明します。

#### <a name="phase-0-build-an-application-package"></a>フェーズ 0:アプリケーションパッケージを作成する

このセクションでは、既存のデスクトップ アプリケーションを、アプリケーション パッケージとしてビルドする方法について説明します。 この段階では、MRT の機能は使用しません。

#### <a name="phase-1-localize-the-application-manifest"></a>フェーズ 1:アプリケーションマニフェストをローカライズする
...
アプリケーションのリソースへの変更を行う前にまず、現在使っているパッケージ化とインストールのテクノロジを、UWP の標準のパッケージ化と展開のテクノロジに置き換える必要があります。 これには、次の 3 つの方法があります。
...
[デスクトップアプリコンバーター](https://aka.ms/converter)を使用する場合は、変換プロセスの詳細について、「[デスクトップアプリコンバーターを使用してデスクトップアプリケーションをパッケージ化](https://aka.ms/converterdocs)する」を参照してください。 デスクトップのコンバーターのサンプルの完全なセットについては[、デスクトップブリッジの UWP サンプル github リポジトリを](https://github.com/Microsoft/DesktopBridgeToUWP-Samples)参照してください。

パッケージを手動で作成する場合は、すべてのアプリケーションのファイル (実行可能ファイルとコンテンツ) とパッケージマニフェストファイル (package.appxmanifest) を含むディレクトリ構造を作成する必要があります。 例は[Hello, World GitHub サンプル](https://github.com/Microsoft/DesktopBridgeToUWP-Samples/blob/master/Samples/HelloWorldSample/CentennialPackage/AppxManifest.xml)にありますが、という`ContosoDemo.exe`デスクトップ実行可能ファイルを実行する基本的なパッケージマニフェストファイルは次のようになります。<span style="background-color: yellow">強調表示</span>されたテキストは、独自の値に置き換えられます。
...
フェーズ0では、アプリケーションの基本的なパッケージマニフェスト (package.appxmanifest) ファイルを作成しました (コンバーターに指定された値に基づいて、MSI から抽出されるか、手動でマニフェストに入力されます) が、ローカライズされた情報は含まれず、サポートされません。高解像度のスタートタイル資産などの追加機能
...
1. プロジェクトに`resources.resw`フォルダー (またはその他の言語) を作成し、既定の名前を使用して、プロジェクトのルートフォルダーに**新しい項目**を追加します。 `Strings\en-us` リソースディクショナリで**はなくリソース** **ファイル (resw)** を選択してください。リソースディクショナリは、XAML アプリケーションによって使用されるファイルです。
2. デザイナーを使って次の文字列を入力します (同じ `Names` を使用し、`Values` をアプリケーションのために適切なテキストに置き換えます)。

<img src="images\editing-resources-resw.png"/>

> [!NOTE]
> Visual Studio デザイナーから始める場合は、を押し`F7`て、いつでも XML を直接編集できます。 ただし、最小限の XML ファイルで開始する場合、さまざまな追加のメタデータが含まれていないため、*デザイナーは、ファイルを認識しません*。手動で編集する XML ファイルに、デザイナーが生成したファイルからスケルトンの XSD 情報をコピーすると、この問題を解決できます。
...
XML を直接編集する場合は、`AppxManifest.xml` ファイルを開き、<span style="background-color: lightgreen">強調表示された値</span>に次の変更を行います。これは、アプリケーション固有ではなく、このテキストを*そのまま*使用します。 これらの正確なリソース名&mdash;を使用する必要はありませんが、独自&mdash;に選択できますが、 `.resw`ファイル内の内容と正確に一致している必要があります。 これらの名前は `.resw` ファイルで作成した `Names` と一致し、`ms-resource:` スキームと `Resources/` 名前空間のプレフィックスが付く必要があります。 
...
### <a name="step-12-build-pri-file-make-an-msix-package-and-verify-its-working"></a>手順 1.2:PRI ファイルをビルドし、MSIX パッケージを作成して、動作していることを確認します。
...
手動でビルドする場合は、次の手順に従ってツールの`MakePRI`構成ファイルを作成し`.pri` 、ファイル自体を生成します (詳細については、「[アプリの手動パッケージング](/windows/msix/package/manual-packaging-root)」を参照してください)。
...
5. 既定の言語リソース (たとえば en-US) を持つ `.pri` が作成されました。 次のコマンドを実行して、正しく動作することを確認します。
    ...
    `makepri dump /?` と入力すると、各パラメーターの意味が表示されますが、主なパラメーターは次のとおりです:
      * `/if`入力ファイル名を設定します。 
      * `/of`出力ファイル名を設定`.xml`します (自動的に追加されます)
      * `/o`出力ファイルが存在する場合に上書きするように設定します
...
1. アプリパッケージを作成するには、次のコマンド`contoso_demo.appx`を実行します。を作成する msix/AppX ファイルの名前に置き換え、ファイル用に別のディレクトリを選択します (このサンプルでは親ディレクトリを使用します。これは任意の場所**にすることができます。** プロジェクトディレクトリではありません)。
...
2. パッケージが作成された後は、署名されている必要があります。 署名証明書を取得する最も簡単な方法は、Visual Studio で空のユニバーサル Windows プロジェクトを作成`.pfx`し、作成したファイルをコピーすることです。 `MakeCert`ただし`Pvk2Pfx` 、「」の説明に[従って、とユーティリティを使用して手動で作成することができます。アプリパッケージ署名証明書を作成する方法](https://docs.microsoft.com/windows/desktop/appxpkg/how-to-create-a-package-signing-certificate)。
...
3. パッケージに署名するには、次のコマンドを使用します。 `AppxManifest.xml` の `Identity` 要素で指定されている `Publisher` は、証明書の `Subject` と一致する必要があります (これは `<PublisherDisplayName>` 要素では**ありません**。それはユーザーに表示されるローカライズされた表示名です)。 通常と同様に、`contoso_demo...` のファイル名をプロジェクトに適した名前で置き換えます。さらに `.pfx` ファイルが現在のディレクトリにないことを確認します (**これは非常に重要です**。そうしない場合、プライベート署名キーを含めて、パッケージの一部として作成されてしまいます)。
    ...
    `signtool sign /?` と入力すると、各パラメーターの意味が表示されますが、主なパラメーターは次のとおりです:
      * `/fd`ファイルダイジェストアルゴリズム (SHA256 は AppX の既定値) を設定します。
      * `/a`最適な証明書が自動的に選択されます
...
6. クリック`Place all certificates in the following store`
...
### <a name="step-13-add-more-supported-languages"></a>手順 1.3:サポートされる言語を追加する

パッケージマニフェストに変更が加えられ、初期`resources.resw`ファイルが作成された後、言語を追加するのは簡単です。
...
この記事では、すべてのローカライズされたリソースのファイル`contoso_demo.exe.mui`名が`contoso.strings.xml`同じであることを前提としています (たとえば、また`en-US` `contoso_strings.dll`は`de-DE`または) が、BCP-47 の名前 (、など) の異なるフォルダーに配置されていることを前提としています。 使用するリソースファイルの数、その名前、ファイル形式/関連 Api などは関係ありません。重要なのは、すべての*論理*リソースが同じファイル名を持つことです (ただし、別の*物理*ディレクトリに配置されています)。 
...
### <a name="step-21-code-changes-to-use-mrt-to-locate-files"></a>手順 2.1:MRT.DLL を使用してファイルを検索するようにコードを変更
...
> [!NOTE]
> アプリに他の目的の`AssemblyResolve`ハンドラーが既にある場合は、リソースを解決するコードを既存のコードと統合する必要があります。
...
## <a name="phase-3-building-resource-packs"></a>フェーズ 3:リソースパックの作成
...
Visual Studio を使用している場合は、ファイル`priconfig.packaging.xml` `priconfig.default.xml`を作成してすべての言語をメインパッケージに作成する方法について、[デバイスにリソースがインストールされているかどうかに関係なく、デバイスにリソースがインストールされている](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))ことを確認してください.
...
### <a name="step-32-signing-the-bundle"></a>手順 3.2:バンドルへの署名
