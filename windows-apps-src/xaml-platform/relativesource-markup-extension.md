...
**注**  XAML での **RelativeSource** の使用法には、意図されている使用法、つまり、バインド式の一部として XAML で [**Binding.RelativeSource**](/uwp/api/windows.ui.xaml.data.binding.relativesource) の値を設定する方法のみが示されています。 ただし、値が [**RelativeSource**](/uwp/api/Windows.UI.Xaml.Data.RelativeSource) のプロパティを設定する場合は、理論的にそれ以外の使用法も可能です。
