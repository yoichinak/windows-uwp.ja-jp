---
title: Windows Hello とコンパニオンデバイスでのロック解除
...
3. バック グラウンド タスクで適切なイベントが発生するまで待機する
    ...
    * CollectingCredential: Windows Hello コンパニオンデバイスが、認証フローを開始するために、PC 側でユーザーの操作/ジェスチャに依存している場合 (たとえば、space キーを押すなど)、このイベントを待機します。
