...
| [Bluetooth に接続されたリモートデバイスからオーディオ再生を有効にする](enable-remote-audio-playback.md)                                                                 | この記事では、 [Audioplaybackconnection](/uwp/api/windows.media.audio.audioplaybackconnection)を使用して、bluetooth 接続されたリモートデバイスがローカルコンピューター上でオーディオを再生できるようにする方法を説明します。これにより、PC を bluetooth スピーカーのように動作させ、ユーザーが電話から音声を聞くことができるようにするシナリオが可能になります。                                                                                                                                                                                                       |
