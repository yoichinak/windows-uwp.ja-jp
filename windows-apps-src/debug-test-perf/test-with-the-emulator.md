...
CPU (使用可能な場合、BIOS で有効にすることができます)

-   ハードウェア依存の仮想化 ([互換性の確認](https://www.microsoft.com/download/details.aspx?id=592))。
...
Windows 10 Mobile 用の Microsoft Emulator には、Visual Studio 2015 以降が必要です。以前のバージョンの Visual Studio との下位互換性はありません。
...
> **重要**  tapper ツールを初めて起動すると、Windows ファイアウォールのプロンプトが表示されます。 3 つのチェック ボックスをすべてオンにし、ツールがファイアウォールを通過できるようにする必要があります。そうしないと、ツールが動作せず、通知も表示されません。
...
> **注**  電話で使用されている SD カードは FAT32 ファイルシステムでフォーマットされているため、32 gb は最大サイズです。
...
新しい Windows 10 フライトに更新した後など、特定の状況では、エミュレーターに関連付けられている仮想ネットワークスイッチが、ユーザーインターフェイスを使用して削除できない状態になることがあります。
...
このコマンド  、エミュレーターに関連付けられているものだけでなく、すべてのネットワークデバイスを削除する**ことに注意**してください。 使用しているコンピューターを再度起動すると、すべてのハードウェア ネットワーク デバイスが自動的に検出されます。
 
...
> XDECleanup **  、** すべてのエミュレーター固有の hyper-v vm が削除され、VM チェックポイントや保存された状態も削除されます。

### <a name="uninstall-windows-10-for-mobile-image"></a>モバイル向け Windows 10 イメージのアンインストール

エミュレーターをインストールすると、モバイル向け Windows 10 の VHD イメージがインストールされ、独自のエントリがコントロール パネルの **[プログラムと機能]** の一覧に示されます。 イメージをアンインストールする場合は、インストールされているプログラムの一覧で**Windows 10 For Mobile イメージ\<バージョン\>** を見つけ、右クリックして、 **[アンインストール]** を選択します。

その後、現在のリリースで、エミュレーターの VHD ファイルを手動で削除する必要があります。 エミュレーターを既定のパスにインストールした場合、VHD ファイルは C:\\Program Files (x86)\\Windows Kit\\10\\エミュレーション\\Mobile\\\<version\>\\になります。
