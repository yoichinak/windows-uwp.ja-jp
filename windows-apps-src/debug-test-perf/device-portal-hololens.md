...
2. デバイスの IP アドレスを検索します。 設定 の下にあるデバイスの IP アドレスを検索します。 **ネットワーク & インターネット > wi-fi > ハードウェアのプロパティ >** ます。 または、「Hey Cortana, what is my IP address?」 (コルタナさん、IP アドレスを教えて) のようにたずねることもできます。
...
> [!IMPORTANT]
> PC がデバイスを見つけられない場合は、`http://127.0.0.1:10080`ではなく、HoloLens デバイスの実際のネットワーク IP アドレスを使用してみてください。
