...
たとえば、アプリの実行可能ファイルとその他のコンテンツを C:\Program\, Files\MyDesktopApp にインストールする、パッケージ化されていない既存のデスクトップアプリがある場合は、 **\<AllowExternalContent**を含むスパースパッケージを作成でき\>マニフェスト内の要素。 アプリのインストールプロセスまたはアプリの初回起動時に、スパースパッケージをインストールし、アプリで使用する外部の場所として C:\Program Files\MyDesktopApp\ を宣言できます。
